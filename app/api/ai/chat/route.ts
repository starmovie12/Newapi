...
- **GET /api/cron/process-queue**: GitHub Cron — heartbeat, stuck recovery, pick 1 item, extract + solve.
...
- extractMovieLinks(url): Movie page scrape → download links
...
- TIMER_DOMAINS: ['gadgetsweb', 'review-tech', 'ngwin', 'cryptoinsights'] — SEQUENTIAL
- TARGET_DOMAINS: ['hblinks', 'hubdrive', 'hubcdn', 'hubcloud', 'gdflix', 'drivehub'] — PARALLEL
- LINK_TIMEOUT_MS: 25000 | OVERALL_TIMEOUT_MS: 50000 | STUCK_TASK_THRESHOLD_MS: 10min | MAX_CRON_RETRIES: 3
...
- Auto-Pilot: Queue items via solve_task

## Common Problems:
- "Links fail" → VPS down / source website changed / env wrong
- "Engine OFFLINE" → GitHub Actions disabled / CRON_SECRET mismatch
- "Task stuck" → solve_task timeout / cron recovery broken
- "Queue pending" → Engine offline / stuck item blocking
- "Low success rate" → Solver broken / VPS overloaded
